...
|---|---|---|
| `test_tui_app_tabs_and_search_focus` | <ul><li>Switch active tab (`Recipes` &rarr; `Agents` &rarr; `Generate`)</li><li>Press `/` to focus the search `TextArea`</li><li>Invoke `action_open_generate()`</li></ul> | Ensures keyboard-driven tab switching and quick search activation seamlessly focus the correct text controls. |
| `test_tui_dropdown_search_overlay` | <ul><li>Focus sort `SearchableSelect` on recipes tab</li><li>Press `/` to open the overlay</li><li>Type `z` to filter options, then press `Enter`</li></ul> | Verifies that searchable dropdown overlays handle incremental typing, title updates, and option selection correctly. |
| `test_tui_fullscreen_toggle_resource_tabs` | <ul><li>Press `m` to maximize and restore the detail pane</li><li>Press `l` or click the maximize button to maximize the log pane</li><li>Press `Esc` to restore standard layout</li><li>Switch tabs to verify auto-reset of fullscreen mode</li></ul> | Confirms that pane maximize/restore toggles function cleanly across keybindings and buttons without state corruption. |
...
| `test_tui_main_keybindings_and_shortcuts` | <ul><li>With a recipe selected, trigger `c`, `e`, `d`, `r`, and `g` actions</li><li>Verify transition to target modal screens</li><li>Press `Esc` to dismiss modals</li></ul> | Guarantees that global keyboard navigation immediately dispatches the appropriate modal screen or tab. |
| `test_tui_log_tab_clear_and_export` | <ul><li>Switch to `tab-logs`</li><li>Press `l` to maximize and `Esc` to restore</li><li>Click `Export Logs` button to save `.log`</li><li>Click `Clear Logs` button to empty the log buffer</li></ul> | Ensures application execution logs can be reviewed in fullscreen, archived to disk, and cleared on demand. |
...
| `test_tui_ctrl_c_double_press_logic` | <ul><li>Trigger first `Ctrl+C` (warning notification)</li><li>Trigger second `Ctrl+C` within timeout (exit)</li></ul> | Guarantees double `Ctrl+C` safety guard prevents accidental termination while allowing intentional exit. |
...
| Test Function | Scenario / User Actions | Assertions & Significance |
|---|---|---|
| `test_tui_help_modal_open_and_dismiss` | <ul><li>Press `?` / `F1` to open `HelpScreen`</li><li>Press `Esc` to dismiss</li></ul> | Verifies that shortcut cheat sheets and usage instructions are always accessible and dismissible. |
...
|---|---|---|
| `test_sort_items` | <ul><li>`alpha_asc`: Case-insensitive ascending order</li><li>`alpha_desc`: Case-insensitive descending order</li></ul> | Guarantees deterministic and intuitive alphabetical ordering of resources. |
